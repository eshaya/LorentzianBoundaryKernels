...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/benchmark_33.pdf}
\caption{Benchmark potential for the $(3,3)$ branch on $S^2\times\CP^2$. The dot marks the stable extremum.}
\label{fig:benchmark33}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/benchmark_15.pdf}
\caption{Benchmark potential for the $(1,5)$ branch. Although it has the same family index as $(3,3)$, its flux distribution moves the minimum and changes the local Hessian.}
\label{fig:benchmark15}
\end{figure}
...
For the illustrative coefficients used in Sec.~\ref{sec:numericaldemo}, $A=B=q=1$ and $C=0.05$, the full two-field masses are those used in the demonstration figures:
\begin{table}[h]
\caption{Full two-radion canonical masses for the benchmark $S^2\times\CP^2$ branches. The optional column $m_{\rm fixed\,V_6}^2$ is the one-field mass obtained after imposing the additional fixed-internal-volume truncation of Eq.~\eqref{eq:Gchichi}. All entries are in the same dimensionless potential units used for the benchmark.}
...
The CMB-scale decoupling variables are
\begin{equation}
\mu_\pm^2\equiv\frac{m_\pm^2}{H_*^2}.
\end{equation}
...
\section{Suggested numerical workflow}
A minimal implementation for each $k$ is:
\begin{enumerate}
\item evaluate the background and canonical kinetic matrix;
\item assemble $\Omega^2(k,\eta)$ for all retained modes;
\item initialize the regular positive-frequency fundamental matrix where $k/a$ exceeds every background scale in the retained system;
\item evolve either $F$ or the Riccati matrix $K$;
\item project onto the physical boundary basis;
\item check positivity of $K_I$;
\item integrate out hidden modes by Eq.~\eqref{eq:schur};
\item calculate the observables in Sec.~\ref{sec:observables}.
\end{enumerate}
The procedure is repeated over a logarithmic $k$ grid and over the compactification parameters or flux integers to be screened.
