...
\begin{abstract}
We formulate a compactification test based on the constrained Lorentzian boundary kernel of the Feynman--DeWitt wave functional. At quadratic order its imaginary part is the inverse covariance of the physical boundary perturbations, so scalar, tensor, and isocurvature spectra and stability conditions follow from one matrix-valued object. Heavy moduli and Kaluza--Klein modes enter through a Schur complement, exposing their decoupling or observable imprint before a full Boltzmann analysis. We recover the standard single-field result and apply the construction to $M_4\times S^2\times\CP^2$, including the $(3,3)$ and $(1,5)$ flux branches, and to $M_4\times S^3\times S^3$ as a control. The method is an acceptability test, not a unique reconstruction of the internal manifold.
...
We formulate the missing interface in terms of the quadratic Lorentzian boundary kernel of the Feynman--DeWitt wave functional.  After the lapse, shift, gauge redundancy, and nondynamical matter variables have been eliminated, the kernel is a matrix on the physical perturbation space.  Its Hermitian imaginary part is the inverse Gaussian covariance of the boundary state.  Thus one matrix contains the information required for the primordial scalar, tensor, and isocurvature spectra.  Hidden fields are removed by a Schur complement, making their decoupling, instability, or residual scale dependence explicit before any transfer-function calculation is performed.
...
We illustrate the method with two ten-dimensional products.  The principal worked example is
\begin{equation}
